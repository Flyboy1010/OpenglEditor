...
Framebuffer::Framebuffer(const FramebufferSpecification& fbDeff)
{
    m_id = -1;
    m_depthAttachment = -1;
    m_deff = fbDeff;
...
void Framebuffer::ReadPixels(unsigned int colorAttachmentIndex, int x, int y, int width, int height, void* pixels)
{
    ...
    glReadPixels(x, y, width, height, GL_RED_INTEGER, GL_INT, pixels);
}
...
void Framebuffer::Create()
{
    ...
    {
        const auto& fbAttachment = m_deff.fbAttachments[i];
        ...
        case FramebufferAttachmentFormat::RGBA8:
            ...
            break;
        case FramebufferAttachmentFormat::RED:
