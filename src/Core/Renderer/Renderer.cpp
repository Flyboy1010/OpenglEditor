...
void Renderer::Init()
{
	...
	rd.quadsIB.Create(6 * rd.MAX_QUADS, quadsIBD);
	...
	rd.quadsShader = std::make_unique<Shader>("Assets/Shaders/quads.glsl");
	...
	rd.linesShader = std::make_unique<Shader>("Assets/Shaders/lines.glsl");
}
...
static void FlushQuads()
{
	if (rd.quadsCount > 0)
	{
		...
		rd.quadsVB.Bind();
		rd.quadsVB.SetData(4 * rd.quadsCount * sizeof(QuadVertex), rd.quadsVD);
...
static void FlushLines()
{
	if (rd.linesCount > 0)
	{
		...
		rd.linesVB.Bind();
		rd.linesVB.SetData(2 * rd.linesCount * sizeof(LineVertex), rd.linesVD);
...
void Renderer::RenderMesh(const Mesh* mesh, const glm::mat4& transform, const glm::mat4& cameraProjection, const glm::mat4& cameraTransform, const glm::vec3& cameraPosition, Shader* shader, const Material& material)
{
	shader->Bind();
	...
	// set material albedo map or color

	if (material.usingAlbedoMap)
	{
	...
	// set material metallic and roughness map

	if (material.usingmetallicRoughnessMap)
	{
	...
	{
		shader->SetUniform1f("u_metallic", material.metallic);
		shader->SetUniform1f("u_roughness", material.roughness);
	}
...
void Renderer::RenderModel(const Model* model, const glm::mat4& transform, const glm::mat4& cameraProjection, const glm::mat4& cameraTransform, const glm::vec3& cameraPosition, Shader* shader)
{
	shader->Bind();
	shader->SetUniformMat4("u_model", transform);
	shader->SetUniformMat4("u_projection", cameraProjection);
	...
	for (int i = 0; i < meshes.size(); i++)
	{
		...
		glDrawElements(GL_TRIANGLES, meshes[i]->ib.GetCount(), GL_UNSIGNED_INT, nullptr);
