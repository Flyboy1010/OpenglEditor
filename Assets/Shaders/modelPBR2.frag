...
float ShadowCalculation(vec4 fragmentPositionLightSpace)
{
    // perform perspective division

    vec3 fragmentPosition = fragmentPositionLightSpace.xyz / fragmentPositionLightSpace.w;

    // transform from [-1, 1] to [0, 1] as the shadow map texture

    fragmentPosition = (fragmentPosition + 1.0) / 2.0;

    // sample the shadow map depth in that position

    float shadowMapDepth = texture(u_shadowMap, fragmentPosition.xy).r;

    // check if it is in shadow by comparing the depth value

    float shadow = (fragmentPosition.z - 0.005 > shadowMapDepth) ? 1.0 : 0.0;
...
void main()
{
    ...
    vec3 lightDirection = normalize(vec3(0.0, 0.0, 0.0) - vec3(35.0, 50.0, 35.0));
